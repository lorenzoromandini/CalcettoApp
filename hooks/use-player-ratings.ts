...
export function usePlayerRatings(matchId: string): UsePlayerRatingsReturn {
  ...
  const setRating = useCallback(async (
    ...
    try {
      const { rating: savedRating } = await upsertRatingAction({
        matchId,
        clubMemberId,
        rating,
        comment,
      ...
      setRatings(prev => {
        ...
        if (existing >= 0) {
          ...
          updated[existing] = {
            ...
            ratingDecimal: savedRating.ratingDecimal,
...
export function useRatingsCounts(matchId: string): UseRatingsCountsReturn {
  ...
  const fetchCounts = useCallback(async () => {
    ...
    try {
      const data = await getRatingsCountAction(matchId)
      setRated(data.rated)
