...
must_haves:
  ...
  artifacts:
    ...
    - path: "components/dashboard/player-evolution-chart.tsx"
      provides: "Multi-line Recharts visualization"
...
<objective>
Create player evolution data aggregation and multi-line chart visualization.

Purpose: Enable users to visualize player performance trends over time (DASH-08).
Output: Evolution data function, React hook, PlayerEvolutionChart component with 3 metrics.
</objective>
...
Design notes:
- Returns goals, assists, and rating per match (3 metrics max per RESEARCH.md)
...
- Follows getPlayerRatingHistory pattern
  ...
  <verify>
    ...
    - `grep "goals.*assists.*rating" lib/db/player-evolution.ts` shows all 3 metrics
  </verify>
  <done>
...
<task type="auto">
  <name>Task 4: Add translations for evolution chart</name>
  <files>messages/it.json, messages/en.json</files>
  <action>
Add translation keys for evolution feature in both Italian and English.

Under `dashboard.evolution` namespace:
- `title`: "Evoluzione giocatore" / "Player Evolution"
- `no_data`: "Dati insufficienti per il grafico" / "Not enough data for chart"
- `goals`: "Gol" / "Goals"
...
- `rating`: "Voto" / "Rating"
  ...
  <done>
Both translation files contain dashboard.evolution namespace with title, no_data, goals, assists, and rating keys.
  </done>
</task>
...
<success_criteria>
- Chart displays goals, assists, and rating trends over time
- Each metric shown as distinct colored line
- Chart only appears when 2+ data points exist
</success_criteria>

<output>
After completion, create `.planning/phases/07-dashboard-leaderboards/07-03-SUMMARY.md`
</output>
