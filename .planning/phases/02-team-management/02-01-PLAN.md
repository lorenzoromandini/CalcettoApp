...
must_haves:
  truths:
    ...
    - "players table exists with roles array and sync_status"
    - "team_invites table exists with token and expiration"
    - "RLS helper functions is_team_admin and is_team_member exist"
    - "Indexes on team_members(team_id, user_id) for performance"
    - "IndexedDB schema extended for team_members and invites"
  artifacts:
    - path: "supabase/migrations/20260215000001_teams_players_invites.sql"
      provides: "Database schema with RLS"
      contains: ["CREATE TABLE teams", "CREATE TABLE team_members", "CREATE TABLE players", "CREATE TABLE team_invites"]
    - path: "lib/db/schema.ts"
      provides: "Extended TypeScript types"
      exports: ["TeamMember", "TeamInvite", "TeamRole"]
...
6. **Create helper functions (SECURITY DEFINER for performance):**
...
7. **RLS Policies:**
   ...
   - Players: SELECT for members, ALL for admins
...
Reference RESEARCH.md for exact SQL patterns. Use soft delete (deleted_at) on teams to preserve match history per research recommendations.</action>
  <verify>Run `supabase migration up` locally and verify tables created with `\dt` in SQL editor. Check RLS policies with `\dp`.</verify>
  <done>Migration applies successfully, all 4 tables exist with RLS enabled, indexes created, helper functions defined.</done>
</task>
...
1. **Update lib/db/schema.ts:**
   ...
   - Update `Player` interface to match database: add surname, nickname, jersey_number, avatar_url, user_id, roles (array)
   - Extend `CalcettoDB` schema with 'team_members' and 'team_invites' stores
   - Add indexes: 'team_members': by-team-id, by-user-id, by-sync-status; 'team_invites': by-team-id, by-token

2. **Update lib/db/index.ts:**
   - In the `openDB` call, increment version number for schema migration
   - Add object store creation for 'team_members' and 'team_invites' in upgrade callback
...
Maintain compatibility with existing 'teams', 'players', 'matches', 'offline_actions' stores. The schema should support the offline action queue pattern from Phase 1.</action>
  ...
  <done>TypeScript types compile, IndexedDB schema includes all team-related entities with proper indexes.</done>
</task>
...
If Supabase CLI generation fails, manually create types based on the schema migration, but prefer generated types for accuracy.</action>
  <verify>types/database.ts exists and exports Database type with all 4 tables properly typed.</verify>
...
<verification>
- Migration file exists and contains all 4 tables
- RLS policies properly restrict access based on team membership
- Helper functions use SECURITY DEFINER for performance
- Indexes exist on team_members(team_id, user_id) and team_invites(token)
- IndexedDB schema updated with new stores
- TypeScript types compile without errors
</verification>

<success_criteria>
1. Database migration creates teams, team_members, players, team_invites tables
2. RLS policies enforce team-based access control
3. IndexedDB schema supports offline storage of team data
4. TypeScript types reflect database schema accurately
5. All tables have sync_status field for offline-first tracking
</success_criteria>
